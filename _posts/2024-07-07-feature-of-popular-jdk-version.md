...
1. Unnamed Variables & Patterns (JEP 456) 未命名变量和模式:
    - 这项特性帮助开发者在需要声明但不使用变量或嵌套模式时，减少错误的机会，提升代码的可读性和可维护性​ (Oracle)​。
- String Templates (Second Preview) (JEP 459) 字符串模板（第二次预览）:
    - 简化了包括运行时计算值的字符串的表达，并提高了由用户提供值的程序的安全性。此特性改善了混合文本和表达式的可读性，并允许创建不通过中间字符串表示的非字符串值​ (Oracle)​​ (InfoWorld)​。
- Implicitly Declared Classes and Instance Main Methods (Second Preview) (JEP 463) 隐式声明类和实例主方法（第二次预览）:
    - 为初学者提供了一种平滑的 Java 编程入门途径，使其在不需要理解为大型程序设计的语言特性的情况下编写其第一个程序​ (Oracle)​。
- Structured Concurrency (Second Preview) (JEP 462) 结构化并发（第二次预览）:
    - 通过引入结构化并发 API，简化了错误处理和取消操作，并增强了并发代码的可观察性，帮助消除常见的取消和关闭风险​ (Oracle)​​ (InfoWorld)​。
- Scoped Values (Second Preview) (JEP 464) 范围值（第二次预览）:
    - 允许在线程之间共享不可变数据，提高了项目的易用性、可理解性、性能和鲁棒性​ (Oracle)​。
- Foreign Function & Memory API (JEP 454) 外部函数与内存 API:
    - 提供了一个 API，使 Java 程序能够与 Java 运行时外部的代码和数据进行交互，提高了易用性、灵活性、安全性和性能​ (Oracle)​​ (InfoWorld)​。
- Vector API (Seventh Incubator) (JEP 460) 向量 API（第七次孵化器）:
    - 引入了向量计算 API，使其在支持的 CPU 架构上能可靠地编译为向量指令，性能优于等效的标量计算​ (Oracle)​​ (InfoWorld)​。
- Class-File API (Preview) (JEP 457) 类文件 API（预览）:
    - 提供了一个标准 API，用于解析、生成和转换 Java 类文件，提高了开发者的生产力​ (Oracle)​​ (InfoWorld)​。
- Launch Multi-File Source-Code Programs (JEP 458) 启动多文件源代码程序:
    - 增强了 Java 应用启动器，使其能够运行由多个 Java 源文件组成的程序，使从小型程序到大型程序的过渡更加平滑​ (Oracle)​​ (InfoWorld)​。
- Stream Gatherers (Preview) (JEP 461) 流收集器（预览）:
    - 增强了 Stream API，支持自定义中间操作，使数据转换更加灵活和高效​ (Oracle)​。
- Region Pinning for G1 (JEP 423) G1 的区域固定:
    - 通过允许在某些 JNI 调用期间进行垃圾收集，减少了延迟，从而改进了性能​ (Oracle)​。。
...
- Foreign Function & Memory API (Incubator) (JEP 412) 外部函数与内存 API（孵化器）:
...
- Pattern Matching for Switch (Preview) (JEP 406) Switch 的模式匹配（预览）:
...
- Strongly Encapsulate JDK Internals (JEP 403) 强封装 JDK 内部 API:
...
- Context-Specific Deserialization Filters (JEP 415) 特定上下文的反序列化过滤器:
...
- Remove the Experimental AOT and JIT Compiler (JEP 410) 移除实验性的 AOT 和 JIT 编译器:
...
- Deprecate the Applet API for Removal (JEP 398) 弃用并计划移除 Applet API:
    - Deprecates the Applet API and plans to remove it in future releases.
    - 将 Applet API 标记为弃用并计划在未来的版本中移除。
- New macOS Rendering Pipeline (JEP 382) 新的 macOS 渲染管道:
...
- Vector API (Second Incubator) (JEP 414) 向量 API（第二次孵化器）:
    - Introduces a new Vector API for vector computations, which can improve performance.
    - 引入新的向量 API，提供矢量计算的支持，可以提高性能。
- Restore Always-Strict Floating-Point Semantics (JEP 306) 恢复严格的浮点数语义:
