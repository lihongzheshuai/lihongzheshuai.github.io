...
| `::` (作用域解析符) | 用于**在类外部定义成员函数**，或**访问命名空间成员**。           | `void Student::introduce() { ... };` （类外定义方法） \\ `std::cout` （访问命名空间 `std` 中的 `cout` 对象） |

注意：在调用**对象**的普通（非静态）方法时，我们使用 `.` (点操作符)，例如 `stu1.introduce();`。作用域解析符 `::` 主要用于类外部定义成员函数，或者访问特定命名空间中的成员（比如 `std::cout` 中的 `std` 就是一个命名空间）。
...
> _注：六级考试对多态的要求通常是“了解概念”，具体的虚函数语法和动态绑定机制可能在更高级别深入考察，但理解其思想对于设计程序非常有帮助。_
